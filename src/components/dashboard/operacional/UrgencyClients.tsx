    ...
    <div className="animate-slide-up delay-4 bg-[rgba(200,0,30,0.04)] border border-[rgba(200,0,30,0.18)] rounded-[14px] overflow-hidden mb-3.5">
      ...
      <div className="grid" style={{ gridTemplateColumns: `repeat(${Math.min(urgencyClients.length, 5)},1fr)` }}>
        {urgencyClients.map((c, i) => (
          <div key={c.id} onClick={() => navigate(`/clientes/${c.id}`)} className={`px-3.5 py-2.5 text-center cursor-pointer card-hover ${i < urgencyClients.length - 1 ? "border-r border-[rgba(200,0,30,0.10)]" : ""}`}>
